...
decode_nj=16
train_set=train
test_sets="dev_sofes dev_gos test" #"dev test"
gmm=tri3b
nnet3_affix=

# The rest are configs specific to this script.  Most of the parameters
# are just hardcoded at this level, in the commands below.
affix=1c   # affix for the LSTM directory name
tree_affix=
train_stage=-10
get_egs_stage=-10
decode_iter=
...
if [ $stage -le 13 ]; then
  ...
  fast-lstm-layer name=fastlstm1 cell-dim=512 delay=-3 $lstm_opts
  fast-lstm-layer name=fastlstm2 cell-dim=512 delay=-3 $lstm_opts
  fast-lstm-layer name=fastlstm3 cell-dim=512 delay=-3 $lstm_opts
  fast-lstm-layer name=fastlstm4 cell-dim=512 delay=-3 $lstm_opts
  fast-lstm-layer name=fastlstm5 cell-dim=512 delay=-3 $lstm_opts
  fast-lstm-layer name=fastlstm6 cell-dim=512 delay=-3 $lstm_opts
  ## adding the layers for chain branch
  output-layer name=output input=fastlstm6 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5
  ...
  output-layer name=output-xent input=fastlstm6 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5
EOF
