...
if [ $stage -le 11 ]; then
  ...
  steps/align_fmllr_lats.sh --nj 16 --cmd "$train_cmd" ${lores_train_data_dir} \
...
if [ $stage -le 13 ]; then
  ...
  relu-batchnorm-dropout-layer name=tdnn2 $opts dim=512 input=Append(-2,0,1)
  relu-batchnorm-dropout-layer name=tdnn3 $opts dim=512
  relu-batchnorm-dropout-layer name=tdnn4 $opts dim=512 input=Append(-1,0,2)
  ...
  relu-batchnorm-dropout-layer name=tdnn7 $opts dim=512
  relu-batchnorm-dropout-layer name=tdnn8 $opts dim=512 input=Append(-3,0,3)
  relu-batchnorm-dropout-layer name=tdnn9 $opts dim=512
  relu-batchnorm-dropout-layer name=tdnn10 $opts dim=512 input=Append(-6,-3,0)

  ## adding the layers for chain branch
  relu-batchnorm-layer name=prefinal-chain $opts input=tdnn10 dim=512
  ...
  relu-batchnorm-layer name=prefinal-xent $opts input=tdnn10 dim=512 target-rms=0.5
...
if [ $stage -le 16 ]; then
  ...
  for data in $test_sets; do
      ...
      steps/nnet3/decode.sh \
          ...
          --nj $nspk --cmd "$decode_cmd"  --num-threads 16 \
