...
affix=1b   # affix for the LSTM directory name
tree_affix=
train_stage=-10
get_egs_stage=-10
decode_iter=
...
if [ $stage -le 13 ]; then
  ...
  output-layer name=output input=fastlstm4 output-delay=$label_delay include-log-softmax=false dim=$num_targets max-change=1.5
  ...
  output-layer name=output-xent input=fastlstm4 output-delay=$label_delay dim=$num_targets learning-rate-factor=$learning_rate_factor max-change=1.5
EOF
